    ...
    public function reservationsShortcode($atts = [])
    {
        ...
        try {
            if ($post_data['pick_up_date']) {
                ...
                ?>
                <form action="<?php echo $brand->publicPackagesFirstStepLink; ?>" method="POST" target="hq-rental-iframe" id="hq-form-init">
                    <input type="hidden" name="pick_up_date" value="<?php echo $pickup_date->format($this->settings->hq_datetime_format); ?>"/>
                    <input type="hidden" name="return_date" value="<?php echo $return_date->format($this->settings->hq_datetime_format); ?>"/>
