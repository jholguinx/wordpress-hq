...
class HQRentalsModelsWorkspotLocations extends HQRentalsBaseModel
{
    ...
    public function saveDetails($data)
    {
        ...
        $spots = array_map(function ($unit) use ( &$show_units, $prefix) {
            return [
                'id' => $unit->id,
                'title' => $unit->f353,
                'cover' => ($prefix . ($unit->f358[0]->uuid ? $this->uuid : '') . '/redirect'),
        ...
        $spots_json = array_map(function ($spot) use (&$available_spots_coordinates_Json, &$unavailable_spots_coordinates_Json, &$rented_spots_coordinates_Json, &$available_from_spots_coordinates_Json, $post_id) {
            switch ($spot['status']) {
                case 'available':
                    if ($spot['coordinates']) {
                        foreach (json_decode($spot['coordinates'])->features as $feature) {
                            if ($feature->properties) {
                                ...
                                caag_setFeatureProperties($feature, $spot, $post_id);
                            }
